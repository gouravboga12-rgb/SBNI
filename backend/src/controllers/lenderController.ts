...
export const getVendorProfiles = async (req: AuthenticatedRequest, res: Response) => {
  ...
  const formatted = vendors.map((v) => {
    ...
    return {
      ...
      shopPhotoUrl: (v as any).shopPhotos ? (() => { try { const p = JSON.parse((v as any).shopPhotos); return p[0] || v.logoUrl || v.avatarUrl; } catch { return v.logoUrl || v.avatarUrl; } })() : (v.logoUrl || v.avatarUrl || null),
      shopPhotos: (v as any).shopPhotos ? (() => { try { return JSON.parse((v as any).shopPhotos); } catch { return [v.avatarUrl]; } })() : (v.avatarUrl ? [v.avatarUrl] : []),
